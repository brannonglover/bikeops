...
export function routeForNotification(
  ...
  if (role === "staff") {
    switch (data.type) {
      ...
      case "missed_call":
        // The ring is long over by the time this arrives, so the useful place
        // to land is the call log, where the missed call and any voicemail it
        // left are both waiting.
        return "/(staff)/calls";
    }
  }
